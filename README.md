...
```mermaid
graph TD
    ISP(("Internet<br>ISP / Uplink"))
    MGMT(("Management Switch<br>Unmanaged GbE<br>192.168.121.0/24"))
    DUTA["DUT-A (USG FLEX 700H)<br>ge1 WAN: DHCP client<br>ge3 LAN: 192.168.1.1/24<br>ge4 MGMT: 192.168.121.205"]
    DUTB["DUT-B (USG FLEX 700H)<br>ge1 WAN: DHCP client<br>ge3 LAN: 192.168.2.1/24<br>ge4 MGMT: 192.168.121.202"]
    PCA["PC-A macOS 本機<br>en0: 192.168.121.100"]
    PCB["PC-B Ubuntu Linux<br>eth0: DHCP client<br>enp2s0: 192.168.121.102"]
    PCC["PC-C Ubuntu Linux<br>eth0: DHCP client<br>enp2s0: 192.168.121.103"]

    ISP ---|"P1 ge1/WAN"| DUTA
    ISP ---|"P1 ge1/WAN"| DUTB
    DUTA ---|"P3 ge3/LAN"| PCB
    DUTB ---|"P3 ge3/LAN"| PCC
    DUTA -.-|"P11 ge4/MGMT"| MGMT
    DUTB -.-|"P11 ge4/MGMT"| MGMT
    MGMT -.- PCA
    MGMT -.- PCB
    MGMT -.- PCC
```
...
前往 [Releases](https://github.com/LordGlenn/ipsec-vpn-test/releases) 頁面下載對應平台的預編譯執行檔：

| 平台 | 檔案 |
|------|------|
| macOS (Apple Silicon) | `ipsec_vpn_test-macOS-arm64.tar.gz` |
| Linux (x86_64) | `ipsec_vpn_test-Linux-x86_64.tar.gz` |
| Windows (x86_64) | `ipsec_vpn_test-Windows-x86_64.zip` |
